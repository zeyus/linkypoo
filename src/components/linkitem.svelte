<script lang="ts">
	...
	const iconIndex =
		'linkIcon' in link && typeof link.linkIcon !== 'undefined'
			? availableIcons.indexOf(link.linkIcon)
			: -1;
	// svelte-ignore state_referenced_locally
	const iconSolidIndex =
		'linkIcon' in link && typeof link.linkIcon !== 'undefined'
			? availableIcons.indexOf(link.linkIcon + 'Solid')
			: -1;
...
<A
	href={link.linkAttrs.href || undefined}
	class="flex w-full flex-col items-center justify-center rounded-xl bg-slate-700 p-4 shadow-2xl hover:bg-slate-600"
>
	...
	<div class="w-full">
		{#if link.subtext && link.subtext.length > 0}
			<p class="mt-1 text-center text-sm text-gray-400">{link.subtext}</p>
		{/if}
	</div>
</A>
